...
export default function PortfolioSection({ onOpenContact }) {
  ...
  return (
    <section id="portfolio" style={{ padding: '110px 0', position: 'relative' }}>
      <div className="container">
        ...
        <ScrollReveal variant="fade-up" delay={50}>
          <div style={{ textAlign: 'center', maxWidth: '800px', margin: '0 auto 3.5rem auto' }}>
            ...
            <h2 style={{ fontSize: 'clamp(2.2rem, 4.5vw, 3.5rem)', fontWeight: 800, marginBottom: '1rem' }}>
              Featured Case Studies & <span className="gradient-text-purple">Projects</span>
            </h2>
            <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem' }}>
              Explore how Paceployt delivers excellence across fintech platforms, custom engineering, corporate consulting, and real estate development.
            </p>
          </div>
        </ScrollReveal>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '0.85rem', marginBottom: '3.5rem', flexWrap: 'wrap' }}>
            {[
              ...
              <button
                ...
                style={{ cursor: 'pointer', padding: '0.6rem 1.4rem', fontSize: '0.85rem' }}
              >
                {f.label}
              </button>
            ))}
        ...
        <div className="grid-2" style={{ gap: '2.5rem' }}>
          {filteredProjects.map((proj, idx) => (
            <ScrollReveal key={proj.id} variant="fade-up" delay={(idx + 1) * 100}>
              <div className="glass-card-elevated" style={{ padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%' }}>
                <div>
                  <div style={{ height: '240px', overflow: 'hidden', position: 'relative' }}>
                    <img src={proj.image} alt={proj.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    <div style={{ position: 'absolute', top: '16px', right: '16px', background: 'rgba(9, 5, 20, 0.9)', backdropFilter: 'blur(8px)', padding: '0.35rem 0.85rem', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 800, color: '#c084fc', border: '1px solid rgba(91, 26, 234, 0.4)' }}>
                      {proj.categoryLabel}
                    </div>
                  </div>

                  <div style={{ padding: '2rem' }}>
                    <div style={{ fontSize: '0.8rem', color: '#c084fc', fontWeight: 700, marginBottom: '0.4rem', letterSpacing: '0.05em' }}>
                      {proj.tech}
                    </div>
                    <h3 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: '0.75rem', lineHeight: 1.3 }}>
                      {proj.title}
                    </h3>
                ...
                <div style={{ padding: '0 2rem 2rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  {proj.liveUrl ? (
                    <a
                      href={proj.liveUrl}
                      target="_blank"
                      ...
                      style={{
                        ...
                        borderRadius: '9999px',
                        background: 'linear-gradient(135deg, #5B1AEA 0%, #7c3aed 100%)',
                        color: '#ffffff',
                        fontSize: '0.85rem',
                        fontWeight: 800,
                        textDecoration: 'none',
